...
std::map<std::string, std::vector<std::string>> SCAN_MAPPING = {
...
void DoInjection() {
    ...
    for (auto& pattern : SCAN_MAPPING[TARGET_NAME]) {
        Log("Doing AoB scan.");
        ...
        for (auto& address : addressesFound) {
            ...
            if (contains(validTypes, typeStr)) {
                ...
                if (TARGET_NAME == "Weapon-Power-Mod" && typeStr == "SB_LIMITBODY_EXCESS_POWER_WEAPON") {
